...
export interface RunJobParams {
  ...
  branchName: string;
  defaultBranch: string; // Added default branch name
}
...
export async function runCloudRunJob(
  octokit: Octokit,
  {
    ...
    branchName,
    defaultBranch, // Destructure default branch name
  }: RunJobParams
): Promise<any> {
      ...
      containerOverrides: [
        {
          env: [
            ...
            { name: "DEFAULT_BRANCH", value: defaultBranch }, // Pass default branch as env var
          ],
        },
      ],
    };
